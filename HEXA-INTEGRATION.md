...
### 4.5 Identidad
- **Usuario (UI embebida + lectura):** las requests al MES llevan el **JWT de usuario de HEXA** (HS256). El MES lo valida con el **mismo `JWT_SECRET`** (compartir como secreto de plataforma) o con un **par de claves dedicado** que HEXA emita para el MES. Extrae `company_id` → resuelve su tenant (`hexa_{slug}_mes`). **✅ Ya implementado y verificado del lado MES** (modo `Auth:Mode=HexaJwt`: valida HS256, mapea `company_id`→tenant y `role`→scopes; rechaza sin token / expirado / firma inválida con 401).
  > ⚠️ **Interop crítico — largo del secreto:** el `JWT_SECRET` compartido debe ser **≥ 32 bytes (256 bits)**. El default de dev de HEXA (`insecure-jwt-secret`, 152 bits) lo **acepta PyJWT** pero el **.NET del MES lo RECHAZA** (Microsoft.IdentityModel exige clave ≥256 bits para HS256, RFC 7518 → `IDX10503 signature validation failed`). **Acción HEXA:** usar un `JWT_SECRET` de ≥32 bytes (recomendable igual por seguridad). Verificado del lado MES: secreto corto → 401; ≥32 bytes → 200 + tenant resuelto.
